...
(function( $ ){
  $.fn.jQCloud = function(word_array, callback_function) {
    ...
    var drawWordCloud = function() {
      ...
      var hitTest = function(elem, other_elems, left, width, top, height){
        // Pairwise overlap detection
        
        
        if ((left < 0) || (left>488-width) || (top<0)){
            return true;
        }
        
        var overlapping = function(a, b){
            
          if (Math.abs(2.0*a.offsetLeft + a.offsetWidth - 2.0*b.offsetLeft - b.offsetWidth) < a.offsetWidth + b.offsetWidth + 2) {
      ...
      $.each(word_array, function(index, word) {
        ...
        if (colors) {
            ...
            $this.append("<span id='" + word_id + "' class='color1w" + weight + "' title='" + (word.title || "") + "'>" + inner_html + "</span>"); 
            colors = false;
        }else{
            var inner_html = word.url !== undefined ? "<a href='" + encodeURI(word.url).replace(/'/g, "%27") + "'>" + word.text + "</a>" : word.text;
            $this.append("<span id='" + word_id + "' class='color2w" + weight + "' title='" + (word.title || "") + "'>" + inner_html + "</span>"); 
            colors = true;
        }
        ...
        while(hitTest(document.getElementById(word_id), already_placed_words, left, width, top, height)) {
            radius += step;
